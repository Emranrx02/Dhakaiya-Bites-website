  ...
  try {
    ...
    if (!customer.reward_ready && isExpired(customer.expires_at)) {
      [customer] = await supabaseRest<RewardCustomer[]>(
        ...
        {
          method: "PATCH",
          body: { stamp_count: 0, cycle_started_at: null, expires_at: null, updated_at: new Date().toISOString() },
          prefer: "return=representation",
        },
      );
    }
    ...
    if (customer.reward_ready) {
      return NextResponse.json({
        ...
        message: "Your free dish is ready. Ask the cashier to redeem it first.",
      });
    }
