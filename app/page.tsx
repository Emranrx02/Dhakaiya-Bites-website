...
export default function Home(){
 ...
 return <main>
  <div className="notice"><span>🍔</span> Fast Bites. Big Flavour. Every Craving, One Place. <a href="tel:+8801603365308">Call 01603 365 308</a></div>
    <header className="header wrap"><a href="#home" className="logo"><img src="/brand/logo.png" alt="Dhakaiya Bites logo"/><div><b>Dhakaiya Bites</b><small>GOOD FOOD • EVERY BITE</small></div></a><div className="header-actions"><Link className="btn btn-small" href="/order">Order <span>↗</span></Link><button className={`menu-toggle ${mobileMenuOpen ? "active" : ""}`} onClick={()=>setMobileMenuOpen(v=>!v)} aria-label="Open menu" aria-expanded={mobileMenuOpen}><span/><span/><span/></button></div><nav className={mobileMenuOpen ? "mobile-open" : ""}><a href="#home" onClick={()=>setMobileMenuOpen(false)}>Home</a><button className="nav-menu-button" onClick={()=>{showMenu(0);setMobileMenuOpen(false)}}>Menu</button><a href="#about" onClick={()=>setMobileMenuOpen(false)}>About</a><a href="#verify" onClick={()=>setMobileMenuOpen(false)}>Staff Verify</a><a href="#contact" onClick={()=>setMobileMenuOpen(false)}>Contact</a></nav></header>
