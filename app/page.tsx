...
export default function Home(){
 ...
 return <main>
  <div className="notice"><span>🍔</span> Fast Bites. Big Flavour. Every Craving, One Place. <a href="tel:+8801603365308">Call 01603 365 308</a></div>
    <header className="header wrap"><a href="#home" className="logo"><img src="/brand/logo.png" alt="Dhakaiya Bites logo"/><div><b>Dhakaiya Bites</b><small>GOOD FOOD • EVERY BITE</small></div></a><nav className="desktop-nav"><a href="#home">Home</a><button className="nav-menu-button" onClick={()=>showMenu(0)}>Menu</button><a href="#about">About</a><a href="#verify">Staff Verify</a><a href="#contact">Contact</a></nav><div className="header-actions"><Link className="btn btn-small" href="/order">Order <span>↗</span></Link><div className="mobile-menu"><button type="button" className="mobile-menu-trigger" aria-label="Open navigation" aria-expanded={mobileMenuOpen} onClick={()=>setMobileMenuOpen(open=>!open)}><span/><span/><span/></button>{mobileMenuOpen&&<><button type="button" className="mobile-menu-backdrop" aria-label="Close navigation" onClick={()=>setMobileMenuOpen(false)}/><nav className="mobile-menu-panel" aria-label="Mobile navigation"><a href="#home" onClick={()=>setMobileMenuOpen(false)}>Home</a><a href="/menu" onClick={()=>setMobileMenuOpen(false)}>Full Menu</a><a href="#about" onClick={()=>setMobileMenuOpen(false)}>About Us</a><a href="#verify" onClick={()=>setMobileMenuOpen(false)}>Staff Verification</a><a href="#contact" onClick={()=>setMobileMenuOpen(false)}>Contact</a></nav></>}</div></div></header>
